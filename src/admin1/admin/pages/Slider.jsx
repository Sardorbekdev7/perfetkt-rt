...
import { deleteSlider, getSlider, postSlider, putSlider } from '../../host/Config'
...
export default function Slider() {
  ...
  const getSliderData=()=>{
      getSlider(cookie.token).then(res=>{
        setData(res.data)
        setLoader(false)
    })
    }
    const deleteData=(id)=>{
      setLoader(true)
      deleteSlider(cookie.token, id).then(res=>{
   ...
   if(edit===null){
    postSlider(cookie.token, event).then(res=>{
    ...
    putSlider(cookie.token, event, edit).then(res=>{
