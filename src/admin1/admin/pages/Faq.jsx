...
import { delDes, deleteFaq, getCourse, getDes, getFaq, postDes, postFaq, putDes, putFaq } from '../../host/Config';
...
export default function Faq() {
  ...
  const deleteData=(res)=>{
    setLoader(true)
    deleteFaq(cookei.token, res).then(res => {
  ...
  const onFinish = (event) => {
    setLoader(true)
    if(change === null) {
      postFaq(cookei.token, event).then(res => {
      ...
      putFaq(cookei.token, event, change).then(res => {
...
<Modal title="" 
       ...
       <div>
        ...
        <p style={{fontSize:'16px'}} dangerouslySetInnerHTML={{__html:sel_data!==null?sel_data.answer_uz:''}}/>
       </div>
       <div>
        <b  style={{fontSize:'18px'}}>Matn (ru)</b>
        <p style={{fontSize:'16px'}}  dangerouslySetInnerHTML={{__html:sel_data!==null?sel_data.answer_ru:''}}/>
       </div>
       <div>
