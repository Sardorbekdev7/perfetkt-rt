...
import { deleteSubCategory, getCategory, getSubCategory, postSubCategory,  putSubCategory } from '../../host/Config'
...
export default function SubCategories() {
  ...
  const getSubCategoryData=()=>{
    ...
    const deleteData=(id)=>{
      setLoader(true)
      deleteSubCategory(cookie.token, id).then(res=>{
   ...
   if(edit===null){
    postSubCategory(cookie.token, event).then(res=>{
    ...
    putSubCategory(cookie.token, event, edit).then(res=>{
    ...
    <div>
        <div className='adminBox'>
            ...
            {!changePage?<>
                ...
                <Button disabled={category===null || category.length===0} style={{marginBottom:'20px'}} onClick={showModal} type='primary'>Kurs turi qo'shish</Button>
                {category===null || category.length===0?<p>Oldin kurs turini qo'shing</p>:<></>}
                </div>
...
<Button disabled={btnDis} style={{marginLeft:'20px', backgroundColor:"#00a900"}} type="primary" htmlType="submit"> Saqlash</Button>
    ...
    >
      ...
      label="Kurs turi"
      name="category_id"
      initialValue={category!==null && category.length!==0?category[0].id:null}
      rules={[
        {
          required: true,
          message: "Bu maydonni to'ldirish shart",
    ...
    >
<Select>
    {category!==null?category.map((item, key)=>{
